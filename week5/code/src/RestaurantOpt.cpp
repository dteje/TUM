...
int main(){
  ...
  for (int testcase=1 ; testcase <= t ; testcase++) {
    int numOfPoints, numOfConnections;
    std::cin >> ;
    std::cin >> numOfConnections;

    PushRelabel<int> pushrelabel (numOfPoints+1);
       ...
       for (int m=0 ; m < numOfConnections; m++)
       {
           ...
           pushrelabel.AddEdge(ori, des, wei);
           pushrelabel.AddEdge(des, ori, wei);
      }

           int result = pushrelabel.GetMaxFlow(1, numOfPoints);

           if(!result) std::cout << "Case #" << testcase << ": impossible\n";
           else std::cout << "Case #" << testcase << ": " << result << "\n";
